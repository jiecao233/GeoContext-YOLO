...
class v8SegmentationLossBoundary(v8SegmentationLoss):
    """在原实例 mask BCE 上增加一个边界辅助项。"""

    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):
        super().__init__(model, tal_topk, tal_topk2)
        self.lambda_boundary = 0.25  # 先固定，后面再消融

    @staticmethod
    def _mask_to_boundary(mask: torch.Tensor, k: int = 3) -> torch.Tensor:
        """
        mask: [N, H, W] or [N, 1, H, W], 值域 0~1
        用 dilation - erosion 近似边界。
        """
        if mask.ndim == 3:
    ...
    def single_mask_loss(
        ...
        """
        在原来的 region BCE 上，加一个 boundary BCE。
        """
        pred_mask = torch.einsum("in,nhw->ihw", pred, proto)  # [N, H, W]

        # 原始区域损失
        region_loss = F.binary_cross_entropy_with_logits(pred_mask, gt_mask, reduction="none")
        region_loss = crop_mask(region_loss, xyxy).mean(dim=(1, 2)) / area

        # 边界辅助损失
        device_type = pred_mask.device.type  # "cuda" or "cpu"

        with torch.amp.autocast(device_type=device_type, enabled=False):
            pred_boundary = self._mask_to_boundary(pred_mask.float().sigmoid())
            gt_boundary = self._mask_to_boundary(gt_mask.float())

            boundary_loss = F.binary_cross_entropy(
                pred_boundary.clamp(1e-6, 1 - 1e-6),
                gt_boundary,
                reduction="none",
            )
            boundary_loss = crop_mask(boundary_loss, xyxy).mean(dim=(1, 2)) / area.float()

        return (region_loss + self.lambda_boundary * boundary_loss).sum()

...
class DCNBNAct(nn.Module):
    """Safer Deformable Conv v2 + BN + SiLU (DCN path forced to FP32)."""
    def __init__(
        self,
        c1: int,
        c2: int,
        ...
        nn.init.constant_(self.mask_conv.bias, 2.0)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out_dtype = x.dtype
        device_type = x.device.type

        # 关键：DCN 这一路全部强制用 float32
        self.offset_conv.float()
        self.mask_conv.float()
        self.conv.float()
        self.bn.float()
        ...
        with torch.amp.autocast(device_type=device_type, enabled=False):
            ...
            offset = self.offset_conv(x_fp32)
            mask = self.mask_conv(x_fp32).sigmoid()

            offset = torch.nan_to_num(offset, nan=0.0, posinf=0.0, neginf=0.0)
            mask = torch.nan_to_num(mask, nan=0.5, posinf=1.0, neginf=0.0).clamp_(0.0, 1.0)

            y = self.conv(x_fp32, offset, mask)
            y = self.bn(y)
            y = self.act(y)
...
class BottleneckDCN(nn.Module):
    """把 Bottleneck 的第二个 3x3 Conv 替换成 DCN."""
    def __init__(
        self,
        c1: int,
        c2: int,
        ...
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DCNBNAct(c_, c2, k=k2, s=1, g=g)
        self.add = shortcut and c1 == c2
...
class C3k_DCN(C3):
    ...
    def __init__(
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(BottleneckDCN(c_, c_, shortcut, g, e=1.0, k2=k) for _ in range(n)))


class C3k2_DCN(C2f):
    """
    保持与官方 C3k2 一致的签名，便于直接在 YAML 里替换。
    """
    def __init__(
        self,
        c1: int,
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                BottleneckDCN(self.c, self.c, shortcut, g),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else C3k_DCN(self.c, self.c, 2, shortcut, g)
            if c3k
...
class LSKA(nn.Module):
    """
    Lightweight Large Selective Kernel Attention.
    这是一个轻量实现，适合直接插到 YOLO26 顶层特征里。
    """
    def __init__(self, dim: int):
        super().__init__()
        hidden = max(dim // 2, 16)

        # 两条深度卷积分支：一个较小核，一个较大有效感受野
        ...
        # 通道压缩后做选择
        self.conv1 = nn.Conv2d(dim, hidden, kernel_size=1)
        self.conv2 = nn.Conv2d(dim, hidden, kernel_size=1)

        # 根据 avg/max 描述生成两分支选择权重
        self.conv_squeeze = nn.Conv2d(2, 2, kernel_size=7, stride=1, padding=3)

        # 恢复到原通道数
        self.conv_out = nn.Conv2d(hidden, dim, kernel_size=1)
...
class C2PSA_LSK(C2PSA):
    """
    保留原 C2PSA 的 PSABlock attention，再在其后叠加一个 LSK block。
    这样原 attention 机制还在。
    """
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5):
        super().__init__(c1, c2, n=n, e=e)
        self.lsk = LSKBlock(self.c)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = self.m(b)      # 原始 C2PSA 的 PSA attention 仍然保留
        b = self.lsk(b)    # 新增 LSK
